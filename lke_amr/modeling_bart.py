...
# -------------------------------------------parameter setting-------------------------------
LENGTH_dep_tag2id = 46
LENGTH_srl_tag2id = 67
DEP_hidden_size = 50
SRL_hidden_size = 50
Feature_size = 20
HIDDEN_size = 1024
...
class AMRBartEncoder(nn.Module):
    ...
    def __init__(self, config: bart.BartConfig, embed_tokens, backpointer_idx):
        ...
        self.srl_edge_embeddings = nn.Embedding(LENGTH_srl_tag2id, SRL_hidden_size)
        ...
        self.ta_gat_layer = MultiHeadGATLayer(TAGATLayer, HIDDEN_size, HIDDEN_size,
                                              Feature_size, GAT_num_heads, DEP_dropout_rate)
        self.ap_gat_layer = MultiHeadGATLayer(APGATLayer, HIDDEN_size, HIDDEN_size,
                                              SRL_hidden_size, GAT_num_heads, DEP_dropout_rate)
        self.pa_gat_layer = MultiHeadGATLayer(PAGATLayer, HIDDEN_size, HIDDEN_size,
                                              SRL_hidden_size, GAT_num_heads, DEP_dropout_rate)
        self.at_gat_layer = MultiHeadGATLayer(ATGATLayer, HIDDEN_size, HIDDEN_size,
                                              Feature_size, GAT_num_heads, DEP_dropout_rate)

        self.linear_dep = self.ffnn(input_size=HIDDEN_size * 2,
                                    num_hidden_layers=0,
                                    hidden_size=-1,
                                    output_size=1,
                                    dropout=None)
        self.linear_srl = self.ffnn(input_size=HIDDEN_size * 2,
                                    num_hidden_layers=0,
                                    hidden_size=-1,
                                    output_size=1,
                                    dropout=None)
    ...
    def forward(
        self, input_ids, embedded=None, attention_mask=None, dep_graphs=None, arg_ids=None,arg_mask=None,predicates=None
    ):
        """
        Args:
        ...
        for index, dep_graph in enumerate(dep_graphs):
            ...
            current_arg_ids = arg_ids[index].to(x.device)
            current_arg_mask = arg_mask[index].to(x.device)
            ...
            if current_arg_ids.size()[0] != 0:
                # [num_arguments, max_arg_len, embed_size]
                arg_embeddings = x_reshape[current_arg_ids] * current_arg_mask.unsqueeze(-1)
                # [num_arguments, embed_size]
                arg_embeddings = torch.sum(arg_embeddings, dim=1) / torch.sum(current_arg_mask, dim=1, keepdim=True)
                predicate_embeddings = x_reshape[predicates[index]]
            ...
            token_embeddings_srl = x_reshape
            if current_arg_ids.size()[0] != 0:
                for i in range(GAT_num_iterations):
            ...
            weight_dep = torch.sigmoid(self.linear_dep(torch.cat([x_reshape, token_embeddings_dep], dim=-1)))
            weight_srl = torch.sigmoid(self.linear_srl(torch.cat([x_reshape, token_embeddings_srl], dim=-1)))
            # [num_tokens, 1, 2]
            weight = torch.softmax(torch.cat([weight_srl, weight_dep], dim=-1), dim=-1).unsqueeze(1)
            # [num_tokens, 2, emb_size]
            enhanced_embeddings = torch.cat([token_embeddings_srl.unsqueeze(1), token_embeddings_dep.unsqueeze(1)],
                                            dim=1)
            enhanced_embeddings = torch.bmm(weight, enhanced_embeddings).squeeze(1)

            f = torch.sigmoid(self.fusion_gate(torch.cat([x_reshape, enhanced_embeddings], dim=-1)))
            mix_embeddings = f * x_reshape + (1 - f) * enhanced_embeddings
...
class AMRBartModel(bart.PretrainedBartModel):
    ...
    def forward(
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(input_ids=input_ids, attention_mask=attention_mask, dep_graphs=dep_graphs,arg_ids=arg_ids,arg_mask=arg_mask,predicates=predicates)
        ...
        # if torch.isnan(decoder_outputs[0][0]).any().item()==True:
        #     print("这里出现了NAN")
...
class AMRBartForConditionalGeneration(bart.PretrainedBartModel):
    ...
    Examples::
        ...
        if lm_labels is not None:
            ...
            # if not lm_labels.is_leaf:
            #     print("这里有问题")
            # print(str(masked_lm_loss))
            # if str(masked_lm_loss.item())=="nan":
            #     print("请注意")
    ...
    def generate(
        ...
        if self.config.is_encoder_decoder:
            ...
            encoder_outputs: tuple = encoder(input_ids, attention_mask=attention_mask, dep_graphs=dep_graphs,arg_ids=arg_ids,arg_mask=arg_mask,predicates=predicates)
